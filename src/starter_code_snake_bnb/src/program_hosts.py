...
def register_cage():
    ...
    if not state.active_account:
        error_msg('You must first log in to register a cage.')
        return
    ...
    price = float(input('How much will you charge per night? '))
...
def list_cages(suppress_header=False):
    ...
    for i, c in enumerate(cages):
        print(f' [{i + 1}] {c.name} is {c.square_meters} meters.')
...
def update_availability():
    ...
    if not state.active_account:
        error_msg('You must first log in to update availability.')
        return
